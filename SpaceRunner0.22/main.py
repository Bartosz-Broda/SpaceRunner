...
class KivyGame(Widget):
    ...
    def init_widgets(self, dt):
        ...
        for _ in range(defs.num_planets):
            planet = Planet(center=(randint(30, w - 30), randint( h, h + 17000)))
            self.planets.append(planet)
            self.add_widget(planet)

        for _ in range(defs.num_rocks):
            rock = Rock(center=(randint(30, w - 30), randint( h, h + 17000)))
    ...
    def init_physics(self):
        ...
        for b in self.planets:
            ...
            for i in range (1):
                b.body.apply_force((0 , randint(-7600, -1600)))

        for b in self.rocks:
            self.init_body(b, defs.rock_collision_type, defs.rock_mass)
            for i in range (1):
                b.body.apply_force((0 , randint(-150, -30)))
